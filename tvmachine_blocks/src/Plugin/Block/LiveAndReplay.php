...
/**
 * Provides a 'Live and replay' block.
 *
 ...
 *
 * @Block(
 *   id = "tvmachine_live_and_replay_block",
 *   admin_label = @Translation("Live and replay")
 * )
 */
class LiveAndReplay extends BlockBase {
  ...
  public function build() {
    ...
    $nids = $DbQueryHelper->get_channels(true, $nids, 2);
    ...
    foreach ($nids as $key => $chnid_load) {
      ...
      if($cha_url!="") {
        ...
        $webuu = '<a href="'.$cha_url.'" target="_blank" class="inner_web_live_and_replay" onclick="window.open.focus(this, this.target, \'scrollbars=yes, menubar=yes, height=700, width=1000, resizable=yes, toolbar=yes, location=yes, status=yes\'); return false;">Web</a>';
      }
      else {$webuu="";}
      ...
      if($cha_live!="") {
        ...
        $live_url= '<a href="'.$cha_live.'" target="_blank" class="inner_live_live_and_replay" onclick="window.open.focus(this, this.target, \'scrollbars=yes, menubar=yes, height=700, width=1000, resizable=yes, toolbar=yes, location=yes, status=yes\'); return false;">En directo</a>';
      }
      else {$live_url="";}
      ...
      if($cha_replay!="")  {
        ...
        $replay_cha = '<a href="'.$cha_replay.'" target="_blank" class="inner_replay_live_and_replay" onclick="window.open.focus(this, this.target, \'scrollbars=yes, menubar=yes, height=700, width=1000, resizable=yes, toolbar=yes, location=yes, status=yes\'); return false;">A la carta</a>';
      }
      else { $replay_cha=""; }
      ...
      if(strpos($title,'blocked') !== false) {        
        ...
        $output .= "<div>
        <div class='logo_live_and_replay' class='tvlogo---".$key."'> <img src='".$ch_image."' alt='".$title."' height='25' width='30'></div>
        <div class='title_live_and_replay'>".$title."</div>
        <div class='web_live_and_replay'>".$webuu."</div>
        <div class='live_live_and_replay'>".$live_url."</div>
        <div class='replay_live_and_replay'>".$replay_cha."</div>
